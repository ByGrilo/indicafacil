...
public class IndicaFacilService {
    ...
    public List<PerfilTrabalhador> listarTrabalhadores(OrdenacaoProfissional ordenacao) {
        OrdenacaoProfissional ordenacaoAplicada = ordenacao == null ? new OrdenacaoPorAvaliacao() : ordenacao;
        return ordenacaoAplicada.ordenar(listarTrabalhadores());
    }
    ...
    public List<PerfilTrabalhador> buscarPorCategoria(CategoriaServico categoria, OrdenacaoProfissional ordenacao) {
        OrdenacaoProfissional ordenacaoAplicada = ordenacao == null ? new OrdenacaoPorAvaliacao() : ordenacao;
        return ordenacaoAplicada.ordenar(buscarPorCategoria(categoria));
    }
    ...
    public Optional<PerfilTrabalhador> buscarMelhorAvaliado() {
        List<PerfilTrabalhador> ranking = listarRanking();
        if (ranking.isEmpty()) {
            return Optional.empty();
        }

        return Optional.of(ranking.get(0));
    }
    ...
    public List<PerfilTrabalhador> buscarPorTexto(String termo, OrdenacaoProfissional ordenacao) {
        OrdenacaoProfissional ordenacaoAplicada = ordenacao == null ? new OrdenacaoPorAvaliacao() : ordenacao;
        return ordenacaoAplicada.ordenar(buscarPorTexto(termo));
    }
    ...
    public List<PerfilTrabalhador> listarFavoritos(long usuarioId, int limite) {
        return favoritoRepository.listarFavoritos(usuarioId, limite);
    }
    ...
    public Optional<AtividadeUsuario> buscarAvaliacaoDoUsuario(long usuarioId, long trabalhadorId) {
        for (AtividadeUsuario atividade : listarAtividadeDoUsuario(usuarioId)) {
            if (atividade.getTrabalhadorId() == trabalhadorId) {
